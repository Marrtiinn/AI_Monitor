...
def generate_chatbot_response(query: str, session: dict) -> str:
    ...
    if api_key_to_use and HAS_GENAI:
        try:
            ...
            # Format context
            context_prompt = f"""
أنت مدرب مبيعات محترف ومستشار جودة المكالمات لشركة Engosoft. تساعد مدققي الجودة ومدراء المبيعات على مراجعة وتحسين أداء الموظفين في المكالمات بناءً على تقرير الجودة المرفق.
...
"""
            response = client.models.generate_content(
                ...
                config=types.GenerateContentConfig(
                    system_instruction="أنت مساعد كواليتي ذكي ومدرب مبيعات خبير في شركة Engosoft. تجيب بلغة عربية احترافية وتقدم أمثلة صياغة ونصوص بيع واقعية وعالية التحويل.",
                    temperature=0.7,
                )
            )
...
4. **دعوة للإجراء (CTA):** "ما رأيك أن نقوم بالتسجيل اليوم ونبدأ بالدفعة الأولى؟" """
    ...
    return f"""مرحباً بك! أنا مساعد الكواليتي الذكي لشركة **Engosoft**.
